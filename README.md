...
*   **Многофункциональный редактор слайдов:**
    ...
    *   Добавление и редактирование текста, изображений, аудио и видео (с YouTube или загруженных с компьютера).
    *   Настройка автовоспроизведения (autoplay) и отключения звука (muted) для медиа-элементов.
...
*   **Продвинутый экспорт в PPTX и PDF:**
    *   Корректное встраивание загруженных видео и аудио, совместимое с PowerPoint.
    *   Сохранение фоновых изображений и цветов заливки для каждого слайда.
    *   Точное сохранение позиций и размеров всех элементов.
*   **Интеграция с Искусственным Интеллектом:**
    *   **Генерация презентации "с нуля":** Создание полноценной презентации на 4-7 слайдов по одной текстовой теме.
    *   **Интерактивный AI-помощник в редакторе:** Помощь в работе над конкретным слайдом (сокращение/улучшение текста, подбор изображений).
*   **Шаблоны:** Создание презентаций на основе готовых шаблонов.
*   **Административная панель:**
    *   Управление шаблонами презентаций.
    *   Управление доступом пользователей к AI-функциям.
    *   **Динамическое управление AI-промптами:** Возможность изменять системные промпты для GigaChat прямо из интерфейса.
...
4.  **Добавьте в PATH:** Вам нужно добавить путь к папке `bin` в системные переменные.
    *   Путь будет выглядеть примерно так: `C:\ffmpeg\ffmpeg-X.X-full_build\bin` (версия может отличаться).
